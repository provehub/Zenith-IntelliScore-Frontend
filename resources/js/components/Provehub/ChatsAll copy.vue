          ...
          >
              <div class="flex justify-between items-center ">
                  <div class="flex items-center">
                      <div class="flex-none">
                        <UserCircleIcon v-if="$page.props.auth.user.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300"  />
                        <img v-else :src="$page.props.auth.user.img" class="rounded-full h-12 w-12 object-cover" />
                      </div>
                      <div class="mx-3">
                              ...
                              <div class="flex-1">
                                  <div class="flex items-center">
                                      <div class="flex-shrink-0 relative">
                                        <UserCircleIcon v-if="person.owner.user.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300"  />
                                          <img v-else :src="person.owner.user.img" class="rounded-full h-12 w-12 object-cover" />
                                          <!-- comeback -->
                                      ...
                                      <div class="mx-auto ">
                                          <p class="mb-1 font-semibold">{{ person.owner.user.name }}</p>
                        ...
                        <!-- if owner -->
                          ...
                          >
                              <div class="flex-1">
                                  <div class="flex items-center">
                                      <div class="flex-shrink-0 relative">
                                        <UserCircleIcon v-if="person.sender.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300"  />
                                          <img v-else :src="person.sender.img" class="rounded-full h-12 w-12 object-cover" />
                  ...
                  <div class="relative h-full">
                      <div v-if="$page.props.auth.user.id !== selectedUser.user2_id" class="flex justify-between items-center p-4">
                          <div class="flex items-center space-x-2 rtl:space-x-reverse">
                              ...
                              <div class="relative flex-none">
                                <UserCircleIcon v-if="selectedUser.owner.user.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300"  />
                                <img v-else :src="selectedUser.owner.user.img" class="rounded-full w-10 h-10 sm:h-12 sm:w-12 object-cover" />
                                  <div class="absolute bottom-0 ltr:right-0 rtl:left-0">
                                      <div class="w-4 h-4 bg-success rounded-full"></div>
                                  </div>
                              </div>
                              <div class="mx-3">
                                  <p class="font-semibold">{{ selectedUser.owner.user.name }}</p>
                          ...
                          <div class="flex sm:gap-5 gap-3">
                            <a v-if="selectedUser.owner_type == 'teachers'" href="#" target="_blank" class="inline-flex items-center gap-2 rounded border border-blue-600 px-4 py-2 bg-blue-600 text-white hover:bg-white hover:text-blue-600 focus:outline-none focus:ring active:bg-blue-500">
                            ...
                            <a v-if="selectedUser.owner_type == 'parents'" href="#" target="_blank" class="inline-flex items-center gap-2 rounded border border-blue-600 px-4 py-2 bg-blue-600 text-white hover:bg-white hover:text-blue-600 focus:outline-none focus:ring active:bg-blue-500">
                            <span class="text-md font-medium">Profile</span>
                            <svg class="h-5 w-5 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                            </svg>
                        </a>
                            <a v-if="selectedUser.owner_type == 'students'" href="#" target="_blank" class="inline-flex items-center gap-2 rounded border border-blue-600 px-4 py-2 bg-blue-600 text-white hover:bg-white hover:text-blue-600 focus:outline-none focus:ring active:bg-blue-500">
                            <span class="text-md font-medium">Profile</span>
                            <svg class="h-5 w-5 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                            </svg>
                      ...
                      <div v-else class="flex justify-between items-center p-4">
                          <div class="flex items-center space-x-2 rtl:space-x-reverse">
                              ...
                              <div class="relative flex-none">
                                <UserCircleIcon v-if="selectedUser.sender.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300"  />
                                <img v-else :src="selectedUser.sender.img" class="rounded-full w-10 h-10 sm:h-12 sm:w-12 object-cover" />
                                  <div class="absolute bottom-0 ltr:right-0 rtl:left-0">
                                      <div class="w-4 h-4 bg-success rounded-full"></div>
                                  </div>
                              </div>
  ...
  <div v-for="message in selectedUser.oldest_messages" :key="message.id" class="flex items-start gap-3" :class="messageAlignment(message.user_id)">
    <div class="flex-none" :class="userClass(message.user_id)">
      <template v-if="isCurrentUser(message.user_id)">
        <UserCircleIcon v-if="selectedUser.sender.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300" />
        <img v-else :src="selectedUser.sender.img" class="rounded-full h-10 w-10 object-cover" />
      </template>
      <template v-if="!isCurrentUser(message.user_id)">
        <UserCircleIcon v-if="selectedUser.receiver.profile_image == null" class="rounded-full h-12 w-12 object-cover text-gray-300" />
        <img v-else :src="selectedUser.receiver.img" class="rounded-full h-10 w-10 object-cover" />
      </template>
    </div>
    <div class="space-y-2">
        ...
        {{ message.time || '5h ago' }}
                ...
                <svg viewBox="0 0 20 20" class="w-6 fill-current">
                    <path d="M11,13 L8,10 L2,16 L11,16 L18,16 L13,11 L11,13 Z M0,3.99406028 C0,2.8927712 0.898212381,2 1.99079514,2 L18.0092049,2 C19.1086907,2 20,2.89451376 20,3.99406028 L20,16.0059397 C20,17.1072288 19.1017876,18 18.0092049,18 L1.99079514,18 C0.891309342,18 0,17.1054862 0,16.0059397 L0,3.99406028 Z M15,9 C16.1045695,9 17,8.1045695 17,7 C17,5.8954305 16.1045695,5 15,5 C13.8954305,5 13,5.8954305 13,7 C13,8.1045695 13.8954305,9 15,9 Z" />
                                  ...
                                  <!-- image upload here -->
                                  <input
                                    class="form-input rounded-full border-0 bg-gray-200 px-12 focus:outline-none py-2 w-full"
...
const props = defineProps({
  ...
  const searchUsers = computed(() => {
    return props.chats.filter(chat =>
        chat.owner && (
            chat.owner.user.name.toLowerCase().includes(searchUser.value.toLowerCase()) ||
