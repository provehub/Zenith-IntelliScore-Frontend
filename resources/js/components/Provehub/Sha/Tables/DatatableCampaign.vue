...
const data = computed(() => props.campaigns?.data || [])
...
const columns = [
  ...
  {
  id: 'duration', // give it a unique id instead of accessorKey
    header: 'Duration',
    cell: ({ row }) => {
      const start = row.original.start_date_formatted || 'N/A'
      const end = row.original.end_date_formatted || 'N/A'
...
<template>
  <Card>
    <CardHeader class="pb-4">
      <CardTitle class="text-base">My Social Media Campaigns</CardTitle>
      <CardDescription>Here you can create and manage your social media campaigns.</CardDescription>
      <NewCampaign />
    </CardHeader>
