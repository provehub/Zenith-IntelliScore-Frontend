  ...
  <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 lg:grid-cols-3 xl:gap-x-8">
    ...
    <div v-for="datum in props.data.data" :key="datum.id" class="rounded-lg bg-gray-50 shadow-sm ring-1 ring-gray-900/5">
      <dl class="flex flex-wrap">
        <div class="flex-auto pl-6 pt-6" v-for="studentclass in datum.studentclass" :key="studentclass.id">
        ...
        <div class="flex-none self-end px-6 pt-4" v-for="studentclass in datum.studentclass" :key="studentclass.id">
        ...
        <div class="mt-4 flex w-full flex-none gap-x-4 px-6" v-for="studentclass in datum.studentclass" :key="studentclass.id">
          <dt class="flex-none">
      ...
      <div class="mt-6 border-t border-gray-900/5 px-6 py-6" v-for="studentclass in datum.studentclass" :key="studentclass.id">
        <a v-if="props.admission.school_class_id != studentclass.id && props.admission.school_class_id != 1" :href="route('view.student.report', { student : studentclass.student_id, studentclass: studentclass.id, term: 1 })" target="_blank" class="text-sm font-semibold leading-6 text-gray-900">First Term Report <span aria-hidden="true">&rarr;</span></a>

        <a v-if="props.admission.school_class_id != studentclass.id && props.admission.school_class_id != 2" :href="route('view.student.report.second.term', { student : studentclass.student_id, studentclass: studentclass.id, term: 2 })" target="_blank" class="text-sm font-semibold leading-6 text-gray-900 ml-4">Second Term Report <span aria-hidden="true">&rarr;</span></a>

        <a v-if="props.admission.school_class_id != studentclass.id && props.admission.school_class_id != 3" :href="route('view.student.report.third.term', { student : studentclass.student_id, studentclass: studentclass.id, term: 3 })" target="_blank" class="text-sm font-semibold leading-6 text-gray-900 ml-4">Third Term Report <span aria-hidden="true">&rarr;</span></a>
